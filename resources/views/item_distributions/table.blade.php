...
<tr class="text-start">
    ...
    <td style="padding:0; margin:0; text-align:center">
        @php
        // Try inventory QR first (non-consumable), then item QR (consumable)
        $qr = $itemDistribution->inventory?->qrCode
        ?? $itemDistribution->item?->qrCode
        ?? null;
        @endphp
        ...
        <br>
        <small>{{ $qr->code }}</small>
        @elseif($itemDistribution->item->type === 'consumable')
        <img src="{{ asset('images/placeholder-qr.png') }}" style="width:40px;">
        <br>
        <small>{{ $itemDistribution->item->name }}</small>
        @else
