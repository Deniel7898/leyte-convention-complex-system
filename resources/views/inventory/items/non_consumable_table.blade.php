@forelse($item->inventories as $unit)
    ...
    <tr>
        ...
        <td style="padding:0; margin:0; vertical-align:top; text-align:center">
            @if($unit->qrCode)
                <img src="{{ asset('storage/' . $unit->qrCode->qr_picture) }}" alt="{{ $unit->qrCode->code }}" width="40"
        ...
        <td class="text-center">
            ...
            @if($status === 'available')
                ...
                <button type="button" title="Issue/Borrow Item"
                    ...
                    data-url="{{ route('item_distributions.create', $unit->id) }}" data-item-id="{{ $item->id }}"
                    data-type="borrowed" data-inventory-id="{{ $unit->id }}" data-quick="1"
                ...
                <button type="button" title="Service Item" class="btn p-0 border-0 bg-transparent text-orange me-2 add-service"
                    data-url="{{ route('service_records.create') }}" data-item-id="{{ $item->id }}"
                    data-inventory-id="{{ $unit->id }}" data-quick="1" data-remaining="{{ $item->remaining ?? 0 }}">
            ...
            @if(in_array($status, ['issued', 'borrowed']) && $item_distribution)
                <button type="button" title="Return Item" class="btn p-0 border-0 bg-transparent text-success me-2 show-return"
            ...
            @if($service_record && in_array($status, ['maintenance', 'installation', 'inspection']))
                <button type="button" title="Complete Service"
                    ...
                    data-url="{{ route('service_records.show_service', $service_record->id) }}" data-unit-id="{{ $unit->id }}">
            ...
            <div class="dropdown d-inline">
                <button class="btn p-0 border-0 bg-transparent text-gray" type="button" id="actionMenu{{ $item->id }}"
                ...
                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="actionMenu{{ $item->id }}">
                    ...
                    <li>
                        <a class="dropdown-item edit text-primary" href="javascript:void(0)"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none"
                                ...
                                class="lucide lucide-eye w-4 h-4 me-2">
                                <path
                    ...
                    <li>
                        <a class="dropdown-item edit-non-consumable text-grey" href="javascript:void(0)"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none"
                                stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                class="lucide lucide-square-pen w-4 h-4 me-2">
                    ...
                    <li>
                        <a class="dropdown-item text-danger delete" href="javascript:void(0)"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none"
                                stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                class="lucide lucide-trash2 w-4 h-4 me-2">
                                <path d="M3 6h18"></path>
