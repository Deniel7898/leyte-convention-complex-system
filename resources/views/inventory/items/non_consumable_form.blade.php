<form action="{{ isset($inventory) ? route('items.update', $inventory->id) : route('inventory.store') }}" method="POST"
            ...
            @if(!isset($item))
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select" id="item-type" name="type" required>
                        ...
                        <option value="consumable" {{ isset($item) && $item->type === 'consumable' ? 'selected' : '' }}>
                            Consumable</option>
