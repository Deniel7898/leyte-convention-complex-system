...
<tr class="text-start">
    ...
    <td style="padding:0; margin:0; vertical-align:top; text-align:center">
        @if($viewItem->qrCode?->qr_picture)
        <img src="{{ asset('storage/' . $viewItem->qrCode->qr_picture) }}"
            alt="{{ $viewItem->qrCode->item->name ?? 'QR Code' }} QR Code"
            width="60"
            class="clickable-img"
            style="cursor: pointer;"
            data-full="{{ asset('storage/' . $viewItem->qrCode->qr_picture) }}">
        @else
        --
        @endif
    </td>
