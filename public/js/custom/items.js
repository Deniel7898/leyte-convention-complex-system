$(function () {
    ...
    $(document).on(
        "click",
        ".edit, .edit-non-consumable, .complete-service, .show-return",
        function () {
            ...
            success: function (response) {
                ...
                if (response.non_consumable_table_html) {
                    $("#items-table-body").html(
                        response.non_consumable_table_html,
                    );
                }
    ...
    $(document).on(
        "click",
        ".add-stock, .add-itemDistribution, .add-service, .add-unit",
        function (e) {
            e.preventDefault();
            ...
            const quick = button.data("quick");
            let inventoryId = button.data("inventory-id");

            // Read remaining stock from data attribute
            // Make sure your buttons have data-remaining="{{ $item->remaining ?? 0 }}"
            let remaining = parseInt(button.attr("data-remaining"), 10) || 0;

            // Check stock for Distribution or Service
            let isDistribution = button.hasClass("add-itemDistribution");
            let isService = button.hasClass("add-service");

            if ((isDistribution || isService) && remaining <= 0) {
                Swal.fire({
                    icon: "warning",
                    title: "Not Allowed",
            ...
            let modalSelector = button.hasClass("complete-service")
                ? "#serviceRecords_modal"
                : "#items_modal";

            // Set modal action
            $(modalSelector).data(
                "action",
                button.hasClass("complete-service") ? "complete" : "update",
            );
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    // Trigger type/item handlers for distribution modal
