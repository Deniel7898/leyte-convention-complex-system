$(function () {
    ...
    $(document).on(
        "click",
        ".add-stock, .add-itemDistribution, .add-service",
        function (e) {
            e.preventDefault();

            let button = $(this).closest(
                ".add-itemDistribution, .add-stock, .add-service",
            );
            let url = button.data("url");
            let itemId = button.data("item-id");
            let type = button.data("type");
            let remaining = Number(button.data("remaining")) || 0;
    ...
    $("#type-filter, #status-filter, #categories-filter").on(
        "change",
        performSearch,
    );
});
